...
Próximo paso: resto de la **Fase 15** — `recipe/` (149), `loot_table/`+`loot_modifiers/` (20), `villager_trade/` (36), y varios directorios menores (`damage_type/`, `data_maps/`, `jukebox_song/`, `neoforge/`, `painting_variant/`, `curios/`) — más adaptación de `PatchouliCompat.java`/libro de guía a la API real de Vellumli (`patchouli_books/`, la parte más compleja, deferida para el final de la fase).
