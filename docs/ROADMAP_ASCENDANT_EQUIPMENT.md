...
Con esto, todo el contenido data-driven de Apotheosis está portado salvo `patchouli_books/` (1 archivo, el libro de guía). Próximo paso: cerrar la **Fase 15** con la adaptación de `PatchouliCompat.java`/libro de guía a la API real de Vellumli — la parte más compleja de la fase, necesita traducción de API real (no solo namespace), referenciando `temp/dependency-src/vellumli/com/skd/vellumli/api/*`.
