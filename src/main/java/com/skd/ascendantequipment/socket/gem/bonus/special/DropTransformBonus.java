...
public class DropTransformBonus extends GemBonus {
   ...
   public DropTransformBonus(
      GemClass gemClass, ContextAwarePredicate conditions, Ingredient inputs, ItemStackTemplate output, Map<Purity, Float> values, String descKey
   ) {
      super(gemClass);
   ...
   public Component getSocketBonusTooltip(GemView inst, AttributeTooltipContext ctx) {
      ...
      return Component.translatable(this.descKey, new Object[]{Affix.fmt(chance * 100.0F)}).withStyle(ChatFormatting.YELLOW);
   }

   @Override
   public void modifyLoot(GemInstance inst, ObjectArrayList<ItemStack> loot, LootContext ctx) {
      if (this.conditions.matches(ctx) && ctx.getRandom().nextFloat() <= this.values.get(inst.purity())) {
   ...
   public static DropTransformBonus.Builder builder() {
      return new DropTransformBonus.Builder();
   }
   ...
   public static class Builder extends GemBonus.Builder {
      ...
      public DropTransformBonus.Builder condition(LootItemCondition condition) {
         this.conditions.add(condition);
         return this;
      }

      public DropTransformBonus.Builder inputs(Ingredient inputs) {
         this.inputs = inputs;
         return this;
      }

      public DropTransformBonus.Builder output(ItemStackTemplate output) {
         this.output = output;
         return this;
      }

      public DropTransformBonus.Builder output(ItemLike item) {
         this.output = new ItemStackTemplate(item.asItem());
         return this;
      }

      public DropTransformBonus.Builder value(Purity purity, float chance) {
      ...
      public DropTransformBonus.Builder desc(String descKey) {
         this.descKey = descKey;
         return this;
      }
