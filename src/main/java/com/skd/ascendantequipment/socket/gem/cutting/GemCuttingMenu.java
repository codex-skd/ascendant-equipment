...
import com.skd.ascendantequipment.socket.gem.cutting.GemCuttingRecipe.CuttingRecipeInput;
...
public class GemCuttingMenu extends CommonToolkitContainerMenu {
    ...
    public boolean stillValid(Player pPlayer) {
        return this.access.evaluate((level, pos) -> level.getBlockState(pos).is(AscEq.Blocks.GEM_CUTTING_TABLE), true);
    }
    ...
    public static List<RecipeHolder<GemCuttingRecipe>> getRecipes(Level level) {
        return level.isClientSide()
            ...
            : List.copyOf(level.getServer().getRecipeManager().getAllRecipesFor(AscEq.RecipeTypes.GEM_CUTTING));
    }
}
