...
public class EquipmentEvents {
    ...
    public void fireProjectile(EntityJoinLevelEvent e) {
        if (e.getEntity() instanceof Projectile proj && !proj.getPersistentData().getBooleanOr("apoth.generated", false) && proj.getOwner() instanceof LivingEntity user) {
    ...
    public void update(Post e) {
        ...
        if (entity.getPersistentData().contains("apoth.burns_in_sun") && entity.level().isBrightOutside() && !entity.level().isClientSide()) {
    ...
    public void clone(Clone e) {
        int oldSeed = e.getOriginal().getPersistentData().getIntOr("apoth_reforge_seed", 0);
    ...
    public void sync(OnDatapackSyncEvent e) {
        ...
        e.sendRecipes(new RecipeType[] { AscEq.RecipeTypes.SALVAGING, AscEq.RecipeTypes.GEM_CUTTING });
        e.getRelevantPlayers().forEach(p -> PacketDistributor.sendToPlayer(p, payload, new CustomPacketPayload[0]));
    }
    ...
    public void recordColdDamage(net.neoforged.neoforge.event.entity.living.LivingDamageEvent.Post e) {
        if (e.getSource().is(AscendantAttributesObjects.DamageTypes.COLD_DAMAGE)) {
            ...
            entity.setData(AscEq.Attachments.COLD_DAMAGE_TAKEN, entity.getData(AscEq.Attachments.COLD_DAMAGE_TAKEN) + e.getInflictedDamage());
        }
    }
    ...
    public void removeCloudsOnDeath(LivingDeathEvent e) {
        if (e.getEntity().getPersistentData().getBooleanOr("apoth.miniboss", false)) {
