...
public final class LootCategory {
   ...
   public static LootCategory forItem(ItemStack stack) {
      ...
      LootCategory override = BuiltInRegistries.ITEM.getData(AscEq.DataMaps.LOOT_CATEGORY_OVERRIDES, stack.getItemHolder().getKey());
      if (override != null) {
         return override;
      }
   ...
   private static ResourceLocation readLocWithApothNamespace(String path) {
      try {
         return path.contains(":") ? ResourceLocation.parse(path) : AscendantEquipment.loc(path);
