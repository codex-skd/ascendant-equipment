...
public class GemLootModifier extends ContextualLootModifier {
   ...
   public GemLootModifier(LootItemCondition[] conditions, List<GemLootModifier.GemTableEntry> entries) {
      super(conditions);
      ...
      for (GemLootModifier.GemTableEntry entry : this.entries) {
         if (entry.pattern.matches(ctx.getQueriedLootTableId())) {
            ...
            if (!entry.gems.isEmpty()) {
               List<WeightedEntry.Wrapper<Gem>> resolved = entry.gems.stream().mapMulti(TieredWeights.wrapFilterHolders(gCtx)).toList();
