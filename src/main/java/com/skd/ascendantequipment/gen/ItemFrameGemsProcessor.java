...
public class ItemFrameGemsProcessor implements StructureProcessor {
   ...
   protected void writeEntityNBT(ServerLevel level, BlockPos pos, RandomSource rand, CompoundTag nbt, StructurePlaceSettings settings) {
      ...
      if (gem != null) {
         ...
         nbt.put("Item", (Tag)ItemStack.CODEC.encodeStart(level.registryAccess().createSerializationContext(NbtOps.INSTANCE), stack).getOrThrow());
      }

      nbt.put("block_pos", (Tag)BlockPos.CODEC.encodeStart(NbtOps.INSTANCE, pos).getOrThrow());
   }
}
