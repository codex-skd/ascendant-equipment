...
public class WorldTierTutorialScreen extends Screen {
   ...
   public void extractBackground(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
      int imgLeft = (this.width - 498) / 2;
      int imgTop = (this.height - 286) / 2;
      gfx.blit(RenderPipelines.GUI_TEXTURED, this.stage.overlay, imgLeft, imgTop, 0.0F, 0.0F, 498, 286, 498, 286);
   }
   ...
   public void extractRenderState(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
      ...
      gfx.text(
         this.font, title.getVisualOrderText(), (int)((imgLeft + 380 - this.font.width(title) * scale / 2.0F) / scale), (int)((imgTop + 107) / scale), -1, true
      );
      pose.popMatrix();
      ...
      for (int i = 0; i < split.size(); i++) {
         ...
         gfx.text(this.font, line, imgLeft + 280, imgTop + 100 + 9 * 3 + (2 + 9) * i, -1, true);
      }
