...
public record GenContext(RandomSource rand, WorldTier tier, float luck, ResourceKey<Level> dimension, Holder<Biome> biome, Set<String> stages) {
   ...
   public static Player findPlayer(LootContext ctx) {
      if (ctx.getParamOrNull(LootContextParams.THIS_ENTITY) instanceof Player p) {
         return p;
      } else if (ctx.getParamOrNull(LootContextParams.ATTACKING_ENTITY) instanceof Player p) {
         return p;
      } else if (ctx.getParamOrNull(LootContextParams.DIRECT_ATTACKING_ENTITY) instanceof Player p) {
         return p;
      } else {
         return ctx.getParamOrNull(LootContextParams.LAST_DAMAGE_PLAYER) != null
            ? (Player)ctx.getParamOrNull(LootContextParams.LAST_DAMAGE_PLAYER)
            : null;
      }
   }
   ...
   public final String toString() {
      return "GenContext[tier=%s, luck=%s, dimension=%s, biome=%s, stages=%s]"
         .formatted(this.tier.getSerializedName(), this.luck, this.dimension.location(), this.biome.getKey().location(), this.stages);
   }
}
