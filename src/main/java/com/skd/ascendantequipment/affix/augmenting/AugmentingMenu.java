...
public class AugmentingMenu extends BlockEntityMenu<AugmentingTableTile> {
         ...
         switch (id & 1) {
            ...
            case 1:
               ...
               if (alternatives.isEmpty()) {
                  ...
                  DynamicHolder<Affix> newAffix = WeightedRandom.getRandomItem(player.getRandom(), weighted)
                     .map(Wrapper::data)
                     .map(AffixRegistry.INSTANCE::holder)
                     .<DynamicHolder<Affix>>orElse(null);
                  if (newAffix == null) {
   ...
   protected static List<WeightedEntry.Wrapper<Affix>> getWeightedAffixes(List<DynamicHolder<Affix>> affixes, GenContext ctx) {
      return affixes.stream().<WeightedEntry.Wrapper<Affix>>mapMulti(TieredWeights.wrapFilterHolders(ctx)).toList();
   }
}
