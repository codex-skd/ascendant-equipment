...
import com.skd.ascendantequipment.affix.salvaging.SalvagingRecipe.OutputData;
...
public class SalvagingMenu extends BlockEntityMenu<SalvagingTableTile> {
    ...
    public boolean stillValid(Player player) {
        ...
        return this.level.getBlockState(this.pos).is(AscEq.Blocks.SALVAGING_TABLE);
    }
    ...
    public static int[] getSalvageCounts(OutputData output, ItemStack stack) {
        ...
        if (stack.isDamageableItem()) {
            ...
            if (maxDmg <= 0) {
                AscendantEquipment.LOGGER.warn("Item {} returned true to ItemStack#isDamageableItem, but returned {} from ItemStack#getMaxDamage, when the value should be positive!", stack.getItemHolder().getKey(), maxDmg);
                return out;
            }
    ...
    public static List<RecipeHolder<SalvagingRecipe>> findMatch(Level level, ItemStack stack) {
        return level.getRecipeManager().getRecipesFor(AscEq.RecipeTypes.SALVAGING, new SingleRecipeInput(stack), level);
    }
