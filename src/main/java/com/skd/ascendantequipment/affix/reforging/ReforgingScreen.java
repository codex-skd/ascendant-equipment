...
public class ReforgingScreen extends AdventureContainerScreen<ReforgingMenu> {
   ...
   public static final ResourceLocation TEXTURE = AscendantEquipment.loc("textures/gui/reforge.png");
   public static final ResourceLocation ANIMATED_TEXTURE = AscendantEquipment.loc("textures/gui/reforge_animation.png");
   ...
   public void render(GuiGraphics gfx, int mouseX, int mouseY, float pPartialTick) {
      ...
      for (int idx = 0; idx < 3; ++idx) {
         ...
         if (this.isHovering(slot.x, slot.y, 16, 16, mouseX, mouseY)) {
            ...
            tooltips.add(Component.translatable("text.ascendant_equipment.reforge_cost").withStyle(ChatFormatting.YELLOW, ChatFormatting.UNDERLINE));
            tooltips.add(CommonComponents.EMPTY);
            if (sigilCost > 0) {
               tooltips.add(Component.translatable("%s %s", sigilCost, new ItemStack(AscEq.Items.SIGIL_OF_REBIRTH).getHoverName()).withStyle(creative || sigils >= sigilCost ? ChatFormatting.GRAY : ChatFormatting.RED));
            }
            if (matCost > 0) {
   ...
   protected void renderBg(GuiGraphics gfx, float partials, int x, int y) {
      ...
      if (!hadItem && this.hasMainItem) {
         ...
         this.minecraft.getSoundManager().play(SimpleSoundInstance.forUI(AscEq.Sounds.REFORGE_ITEM_PLACED.value(), 1F, 2F));
      }
   ...
   public void renderSlot(GuiGraphics gfx, Slot slot) {
      if (slot instanceof ReforgingMenu.ReforgingResultSlot) {
         if (this.animationTick == 0) {
            float alpha = this.maxSlot >= slot.getContainerSlot() ? this.availableOpacity / 255.0F : 64 / 255.0F;
            PipelinedRenderer.ghostFakeItem(gfx, slot.getItem(), slot.x, slot.y, alpha);
         }
      } else {
