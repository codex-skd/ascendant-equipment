...
public class ReforgingMenu extends BlockEntityMenu<ReforgingTableTile> {
   ...
   protected void updateSeed() {
      int seed = this.player.getPersistentData().getIntOr("apoth_reforge_seed", 0);
      if (seed == 0) {
   ...
   public class ReforgingResultSlot extends ResourceHandlerSlot {
      ...
      public void onTake(Player player, ItemStack stack) {
         ...
         player.playSound(AscEq.Sounds.REFORGE_ITEM_REFORGED, 0.25F, player.getRandom().nextFloat() * 0.15F + 1.0F);
      }
   }
}
