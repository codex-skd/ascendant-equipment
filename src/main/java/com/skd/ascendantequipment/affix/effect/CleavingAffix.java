   ...
   public void doPostAttack(AffixInstance inst, LivingEntity user, Entity target) {
      if (AscendantAttributes.getLocalAtkStrength(user) >= 0.98 && !cleaving && !user.level().isClientSide()) {
         ...
         if (user.level().getRandom().nextFloat() < chance && user instanceof Player player) {
            for (Entity e : target.level().getEntities(target, new AABB(target.blockPosition()).inflate(6.0), cleavePredicate(user, target))) {
               if (targets > 0) {
   ...
   public static Predicate<Entity> cleavePredicate(Entity user, Entity target) {
      return e -> {
         if ((!(e instanceof Animal) || target instanceof Animal) && (!(e instanceof AbstractVillager) || target instanceof AbstractVillager)) {
            return (EquipmentConfig.cleaveHitsPlayers || !(e instanceof Player)) && (!(target instanceof Enemy) || e instanceof Enemy)
               ? e != user && e instanceof LivingEntity le && le.isAlive()
               : false;
         } else {
            return false;
         }
