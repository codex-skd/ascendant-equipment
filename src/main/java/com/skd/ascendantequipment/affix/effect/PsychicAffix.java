...
public class PsychicAffix extends Affix {
   ...
   public float onShieldBlock(AffixInstance inst, LivingEntity entity, DamageSource source, float amount) {
      if (source.getDirectEntity() instanceof Projectile arrow && arrow.getOwner() instanceof LivingEntity living && living.level() instanceof ServerLevel sl) {
         living.hurtServer(sl, entity.damageSources().source(AscEq.DamageTypes.PSYCHIC, entity), amount * this.getTrueLevel(inst));
      }
