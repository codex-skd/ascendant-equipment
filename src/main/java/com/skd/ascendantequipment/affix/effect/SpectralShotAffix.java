...
public class SpectralShotAffix extends Affix {
   public static final Codec<SpectralShotAffix> CODEC = RecordCodecBuilder.create(
      inst -> inst.group(affixDef(), LootRarity.mapCodec(StepFunction.CODEC).fieldOf("values").forGetter(a -> a.values)).apply(inst, SpectralShotAffix::new)
   );
   ...
   public void onProjectileFired(AffixInstance inst, LivingEntity user, Projectile proj) {
      if (user.level().getRandom().nextFloat() <= this.getTrueLevel(inst.getRarity(), inst.level())
         ...
         spectralArrow.shoot(user.getXRot(), user.getYRot(), 0.0, 2.0F, 1.0F);
   ...
   private void cloneMotion(AbstractArrow src, AbstractArrow dest) {
      dest.setDeltaMovement(src.getDeltaMovement().scale(1.0));
