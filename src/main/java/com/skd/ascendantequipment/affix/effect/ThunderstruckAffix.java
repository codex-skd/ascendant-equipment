...
public class ThunderstruckAffix extends Affix {
   ...
   public MutableComponent getDescription(AffixInstance inst, AttributeTooltipContext ctx) {
      return Component.translatable("affix." + this.id() + ".desc", new Object[]{fmt(this.getTrueLevel(inst.getRarity(), inst.level()))});
   }
   ...
   public Component getAugmentingText(AffixInstance inst, AttributeTooltipContext ctx) {
      ...
      Component minComp = Component.literal(fmt(this.getTrueLevel(rarity, 0.0F)));
      Component maxComp = Component.literal(fmt(this.getTrueLevel(rarity, 1.0F)));
   ...
   public void doPostAttack(AffixInstance inst, LivingEntity user, Entity target) {
      if (!user.level().isClientSide()) {
         if (AscendantAttributes.getLocalAtkStrength(user) >= 0.98) {
            for (Entity e : target.level().getEntities(target, new AABB(target.blockPosition()).inflate(6.0), CleavingAffix.cleavePredicate(user, target))) {
