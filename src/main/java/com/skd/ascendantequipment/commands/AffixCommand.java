         ...
         if (!held.isEmpty()) {
            ...
            if (!cat.isNone() && rarity.isBound()) {
               Stream<String> suggestions = AffixRegistry.INSTANCE
                  .getValues()
                  .stream()
                  .filter(a -> a.canApplyTo(held, cat, rarity.get()))
   ...
   public static void register(LiteralArgumentBuilder<CommandSourceStack> root) {
      ...
      builder.then(
         Commands.literal("apply")
            .then(
               ((RequiredArgumentBuilder)Commands.argument("affix", ResourceLocationArgument.id())
                  ...
                  .executes(c -> applyAffix(c, ResourceLocationArgument.getId(c, "affix"), ((CommandSourceStack)c.getSource()).getLevel().getRandom().nextFloat()))
            )
      );
      ...
      builder.then(
         Commands.literal("list_alternatives")
            .then(
               Commands.argument("affix", ResourceLocationArgument.id())
   ...
   public static int listAlternatives(CommandContext<CommandSourceStack> c, ResourceLocation affixId) throws CommandSyntaxException {
      ...
      if (((CommandSourceStack)c.getSource()).getEntity() instanceof LivingEntity living) {
         ...
         Stream<DynamicHolder<Affix>> alternatives = LootController.getAlternativeAffixes(
            ((CommandSourceStack)c.getSource()).getPlayerOrException(), held, rarity.get(), afx
         );
         ((CommandSourceStack)c.getSource())
            .sendSystemMessage(Component.translatable("Possible alternatives to %s:", new Object[]{afx.get().getName(true)}));
         AttributeTooltipContext ctx = AttributeTooltipContext.of(
            living instanceof Player p ? p : null,
            TooltipContext.of(((CommandSourceStack)c.getSource()).getLevel()),
            AscendantAttributes.getTooltipFlag()
         ...
         alternatives.forEach(a -> {
            MutableComponent name = Component.translatable("[%s]", new Object[]{a.get().getName(true)});
