...
public record Invader(BasicBossData basicData, EntityType<?> entity, AABB size, Map<LootRarity, BossStats> stats)
   ...
   public static final String INVADER_ATTR_PREFIX = "ascendant_invader_";
   ...
   protected ResourceLocation createAttributeModifierId(int index) {
      ...
      return ResourceLocation.fromNamespaceAndPath(key.getNamespace(), "apothic_invader_" + key.getPath() + "_modif_" + index);
   }
   ...
   public static ItemStack modifyBossItem(ItemStack stack, Component bossName, GenContext ctx, LootRarity rarity, int enchLevel, RegistryAccess reg) {
      ...
      Component bossOwnerName = Component.translatable(NameHelper.ownershipFormat, new Object[]{bossName});
      ...
      if (name != null && name.getContents() instanceof TranslatableContents tc) {
         ...
         RegistryFriendlyByteBuf rfbb = new RegistryFriendlyByteBuf(io.netty.buffer.Unpooled.buffer(), reg, ConnectionType.NEOFORGE);
      ...
      if (EquipmentConfig.curseBossItems) {
         List<Holder.Reference<Enchantment>> curses = reg.lookupOrThrow(Registries.ENCHANTMENT)
            .listElements()
