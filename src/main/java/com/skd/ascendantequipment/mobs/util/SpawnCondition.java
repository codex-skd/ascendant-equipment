...
public interface SpawnCondition extends CodecProvider<SpawnCondition> {
   ...
   boolean test(Mob var1, ServerLevelAccessor var2, EntitySpawnReason var3, @Nullable CompoundTag var4);

   default boolean requiresNbtAccess() {
      return false;
   }

   static boolean checkAll(List<SpawnCondition> conditions, Mob mob, ServerLevelAccessor level, EntitySpawnReason type) {
   ...
   record SpawnTypeCondition(Set<EntitySpawnReason> types) implements SpawnCondition {
      public static Codec<SpawnCondition.SpawnTypeCondition> CODEC = RecordCodecBuilder.create(
         inst -> inst.group(
               CommonToolkitCodecs.setOf(CommonToolkitCodecs.enumCodec(EntitySpawnReason.class)).fieldOf("spawn_types").forGetter(SpawnCondition.SpawnTypeCondition::types)
            )
      ...
      public static SpawnCondition.SpawnTypeCondition of(EntitySpawnReason... types) {
